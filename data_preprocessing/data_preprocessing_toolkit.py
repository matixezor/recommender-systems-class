...
class DataPreprocessingToolkit(object):
    ...
    def sum_npeople(df):
        ...
        # Write your code here #
        ########################
        df["n_people"] = np.maximum(df["n_people"] + df["n_children_1"] + df["n_children_2"] + df["n_children_3"], 1)
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        group_reservations = aggregated_datasets.pop(0)
        for aggregated_dataset in aggregated_datasets:
            group_reservations = group_reservations.merge(aggregated_dataset, on="group_id")
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        # Write your code here #
        ########################
        night_prices = df[df["accommodation_price"] > 1] \
        ...
        df["room_night_price"].fillna(0.0)
        df["room_segment"] = df["room_night_price"].apply(
            lambda x: self.map_value_to_bucket(x, self.room_segment_buckets)
        )
        
        return df.drop(columns=["room_night_price"])
